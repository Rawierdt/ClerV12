...
module.exports.run = async(client, message, args) => {
    ...
    const embedDatos = new Discord.MessageEmbed() 
        ...
        .setImage("https://i.ibb.co/vDF7tNM/ClerGif.gif")
        .setFooter("¿Problemas con los comandos o bugs?, ve al servidor de soporte c!suport | Desarrollado por NexDrak#9439")
        .setTimestamp()
        ...
        .addField("「🐼」• Comandos de Divertidos", "`8ball` `ascii` `comandos` `say` `say embed` `isay |imagen url|` `tweet` `love` `memide` `jumbo` `impostor` `meme` `chiste` `carta` `guapo` `hack` `moneda` `reverse` `basura` `rip`")  
        .addField("「🌸」• Comandos de Utileria", "`avatar` `wallpaper` `invite` `emojis` `serverinfo` `support` `ping` `donate` `hola` `md` `tts` `en_tts` `ro_tts` `ko_tts` `ja_tts` `po_tts` `help` `randomuser` `seeon` `servericon` `botinfo` `iduser` `iav` `about` `sugerencia |tema| |sugerencia|`") 
